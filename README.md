...
### Аутентификация
Логин работает через таблицу `users` в БД (email + BCrypt-хэш пароля), роли берутся из `roles`/`t_roles_users`:
- Роль `admin` даёт доступ к добавлению/редактированию/удалению новостей (`@RolesAllowed("admin")`).
- Новые пользователи регистрируются на странице `/register` и получают роль `user`.
- Первый администратор создаётся в БД при старте приложения из `ADMIN_USERNAME` (используется как email/логин) и `ADMIN_PASSWORD_HASH`. Операция идемпотентна: если пользователь с таким email уже есть, ничего не меняется. Если переменные не заданы, бутстрап пропускается.
